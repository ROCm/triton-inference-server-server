...
After launching Triton using the ``docker run`` command, you should see the model repository load successfully in the server logs.

To verify that Triton Inference Server is installed correctly, run the following:

.. code-block:: bash

   python -c "import triton-inference-server-server; print(triton-inference-server-server.__version__)"
