...
3. Use the python script or ``perf_analyzer`` tool from the `https://github.com/AMD-Ecosystem/triton-inference-server-server <https://github.com/AMD-Ecosystem/triton-inference-server-server>`__ repository to send client requests for end-to-end benchmark tests.
